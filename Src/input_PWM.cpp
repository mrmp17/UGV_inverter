...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
  if(GPIO_Pin == PWM_IN_1_Pin){
    if(HAL_GPIO_ReadPin(PWM_IN_1_GPIO_Port, PWM_IN_1_Pin)==GPIO_PIN_SET){ //rising edge
      ...
      //input.pwm_rising_time[PWM1] = __HAL_TIM_GET_COUNTER(TIMER_USED);
    }
    if(HAL_GPIO_ReadPin(PWM_IN_1_GPIO_Port, PWM_IN_1_Pin)==GPIO_PIN_RESET){ //falling edge
      ...
      //input.pwm_falling_time[PWM1] = __HAL_TIM_GET_COUNTER(TIMER_USED);
      //input.pwm_width[PWM1] = input.pwm_falling_time-input.pwm_rising_time;
    }
  }
